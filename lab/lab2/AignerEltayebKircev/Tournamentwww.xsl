...
>
	...
	<xsl:template match="/tournament">
		<html>
			...
			<body>
				...
				<xsl:for-each select="group/archer">
					<h2>
						<a name="scores">
					...
					<xsl:variable name="archerId" select="@id"/>
					<table border = "1px">
						<tr><th>target</th><th>arrow 1</th><th>arrow 2</th><th>arrow 3</th><th>arrow 4</th></tr>
						<xsl:apply-templates select = "../day/scores[@archer-id = $archerId]/target-scores"/>
					</table>
				</xsl:for-each>
	...
	<xsl:template match="target-scores">
		<tr>
			<td><xsl:value-of select="@target"/></td>
			<td><xsl:value-of select="arrow1-score"/></td>
			<td><xsl:value-of select="arrow2-score"/></td>
			<td><xsl:value-of select="arrow3-score"/></td>
			<td><xsl:value-of select="arrow4-score"/></td>
		</tr>
	</xsl:template>
