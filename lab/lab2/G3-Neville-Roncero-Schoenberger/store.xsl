...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="product" mode="detail">
		...
		<p><b>Artist: </b><xsl:value-of select="id(@artist_id)/@artist_name"></xsl:value-of></p>
		<p><b>Media: </b><xsl:value-of select="@media"></xsl:value-of></p>
		<p><b>Genre: </b><xsl:value-of select="@genre"></xsl:value-of> </p>
		<p><b>Label: </b><xsl:value-of select="@label"></xsl:value-of> </p>
		<p><b>Length: </b><xsl:value-of select="@length"></xsl:value-of> </p>
	...
	<xsl:template match="song" mode="detail">
		<tbody>
			<tr>
				<td> <p id="{generate-id()}"><xsl:value-of select="./@name"/></p> </td>
				<td> 
	...
	<xsl:template match="product/song" mode="all">
		<li>
			...
			<i> in: <xsl:value-of select="ancestor::product/@title"/><br/></i>
		</li>	
	</xsl:template>
