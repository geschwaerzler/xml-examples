			...
			<xsl:for-each select="team">
				...
				<xsl:for-each select="competitor">
					...
					<h5 id= "{@id}">
						...
						<xsl:value-of select="concat('Gegner: ', @association, ' - Geschlecht: ',@gender , ' - Kategorie: ',@type)" xml:space="preserve"/><br></br>
					</h5>
				</xsl:for-each>	
				...
				<xsl:for-each select="match">
					...
					<table cellspacing = "10" cellpadding = "20" border = "thin"> 
						...
						<xsl:for-each select ="//winner-team/scoreboard">
							<tr>
								<td><xsl:value-of select="@player-id"/></td>
					...
					<table cellspacing="10" cellpadding="20" border ="thin"> 
						...
						<xsl:for-each select ="//loser-team/scoreboard">
							<tr>
								<td><xsl:value-of select="@competitor-id"/></td>
								<td><xsl:value-of select="shooting-style/@description"/></td>
								<td><xsl:value-of select="shooting-style/@point"/></td>    
							</tr> 
						</xsl:for-each>
